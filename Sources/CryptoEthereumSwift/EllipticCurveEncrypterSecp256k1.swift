...
public final class EllipticCurveEncrypterSecp256k1 {
    ...
    public func sign(hash: Data, privateKey: Data) -> secp256k1_ecdsa_recoverable_signature? {
        ...
        let status = privateKey.withUnsafeBytes { (keyPointer: UnsafeRawBufferPointer) -> Int32 in
            let key = keyPointer.baseAddress!.assumingMemoryBound(to: UInt8.self)
            return hash.withUnsafeBytes { (hashPointer: UnsafeRawBufferPointer) -> Int32 in
                let hash = hashPointer.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return secp256k1_ecdsa_sign_recoverable(context, &signature, hash, key, nil, nil)
            }
        }
    ...
    public func export(signature: inout secp256k1_ecdsa_recoverable_signature) -> Data {
        ...
        output.withUnsafeMutableBytes { (outputPointer: UnsafeMutableRawBufferPointer) in
            let baseAddress = outputPointer.baseAddress!.assumingMemoryBound(to: UInt8.self)
            secp256k1_ecdsa_recoverable_signature_serialize_compact(context, baseAddress, &recid, &signature)
        }
    ...
    public func `import`(signature: Data) -> secp256k1_ecdsa_recoverable_signature {
        ...
        signature.withUnsafeBytes { (inputPointer: UnsafeRawBufferPointer) -> Void in
            let input = inputPointer.baseAddress!.assumingMemoryBound(to: UInt8.self)
    ...
    public func export(publicKey: inout secp256k1_pubkey, compressed: Bool) -> Data {
        ...
        output.withUnsafeMutableBytes { (pointer: UnsafeMutableRawBufferPointer) -> Void in
            let mutablePointer = pointer.baseAddress!.assumingMemoryBound(to: UInt8.self)
            secp256k1_ec_pubkey_serialize(context, mutablePointer, &outputLen, &publicKey, compressedFlags)
        }
